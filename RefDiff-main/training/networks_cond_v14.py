...
#@persistence.persistent_class
class UNetBlock_Ref(torch.nn.Module):
    def __init__(self,
        in_channels, out_channels, emb_channels, up=False, down=False, attention=False,
        num_heads=None, channels_per_head=64, dropout=0, skip_scale=1, eps=1e-5,
        resample_filter=[1,1], resample_proj=False, adaptive_scale=True,
        ...
        self.norm0 = GroupNorm(num_channels=in_channels, eps=eps)
        self.sft0 = SFTLayer(nf=in_channels, n_condition=4)
        self.conv0 = Conv2d(in_channels=in_channels, out_channels=out_channels, kernel=3, up=up, down=down, resample_filter=resample_filter, **init)
        self.affine = Linear(in_features=emb_channels, out_features=out_channels*(2 if adaptive_scale else 1), **init)
        self.norm1 = GroupNorm(num_channels=out_channels, eps=eps)
        self.sft1 = SFTLayer(nf=out_channels, n_condition=4)
        self.conv1 = Conv2d(in_channels=out_channels, out_channels=out_channels, kernel=3, **init_zero)
    ...
    def forward(self, x, emb, lr_cond, ref_cond, mask_cond):
        orig = x
        x = self.norm0(x)
        x = self.sft0(x, torch.cat([lr_cond,mask_cond], dim=1)) 
        x = self.conv0(silu(x))
   
        x = self.norm1(x)
        x = self.sft1(x, torch.cat([ref_cond,mask_cond], dim=1))
...
class SongUNet(torch.nn.Module):
    ...
    def forward(self, x, noise_labels, lr_cond, ref_cond, mask_cond, class_labels, augment_labels=None):
        # Mapping.
        ...
        # Encoder.
        skips = []
        x = torch.cat([x, lr_cond, ref_cond, mask_cond], dim=1)
        aux = x
        ...
        for name, block in self.dec.items():
            if 'aux_up' in name:
                ...
                x = block(x, emb, lr_cond, ref_cond, mask_cond)
        return aux
...
class EDMPrecond(torch.nn.Module):
    def __init__(self,
        ...
        **model_kwargs,                     # Keyword arguments for the underlying model.
        ...
        #drm modify
        self.model = globals()[model_type](img_resolution=img_resolution, in_channels=10, out_channels=img_channels, label_dim=label_dim, **model_kwargs)

    def forward(self, x, sigma, lr_cond, ref_cond, mask_cond, class_labels=None, force_fp32=False, **model_kwargs):
        x = x.to(torch.float32)
        #drm modify
        lr_cond = lr_cond.to(torch.float32)
        ref_cond = ref_cond.to(torch.float32)
        mask_cond = mask_cond.to(torch.float32)
        ...
        F_x = self.model((c_in * x).to(dtype), c_noise.flatten(), lr_cond, ref_cond, mask_cond, class_labels=class_labels, **model_kwargs)
        #F_x = self.model((c_in * x).to(dtype), c_noise.flatten(), lr_cond, ref_cond, mask_cond, class_labels=class_labels)
        assert F_x.dtype == dtype
        D_x = c_skip * x + c_out * F_x.to(torch.float32)
        return D_x
